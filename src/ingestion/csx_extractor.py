        ...
        for paper in papers:
            try:
                if (paper.pub_info.year):
                    paper.title = "The duty to secure independent and impartial careers guidance for people in schools"
                    ...
                    if (result!=None):
                        if len(result) > 0:
                            result[0]
                            CSXExtractorImpl().mergeMatchingDocs(wrapper, paper, result[0])
                        else:
